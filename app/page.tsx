            ...
            <article className="study-card">
              ...
              <div className="study-toolbar">
                ...
                <div className="speech-controls">
                  <span className="speech-voice" title="使用古诗文读音校正表预先生成">晓晓 · 校音版</span>
                  <label>
                    <span>语速</span>
                    <select value={speechRate} onChange={(event) => {
                      const nextRate = Number(event.target.value);
                      setSpeechRate(nextRate);
                      if (audioRef.current) audioRef.current.playbackRate = nextRate;
                    }} aria-label="选择朗读语速">
                      <option value={0.82}>慢速</option>
                      <option value={1}>标准</option>
                      <option value={1.12}>稍快</option>
                    </select>
                  </label>
                  <button className={isSpeaking ? "speaking" : ""} onClick={speak} aria-label={isSpeaking ? "停止朗读" : "朗读本组诗文"}>
                    {isSpeaking ? "■ 停止" : audioError ? "↻ 重试音频" : "▷ 听朗读"}
                  </button>
                  <audio
                    key={current.id}
                    ref={audioRef}
                    ...
                    onError={() => { setAudioError(true); setIsSpeaking(false); }}
                  />
                </div>
              </div>
